...
DTYPES = row_parser.get_colname_dtypes()

def has_valid_length(row, n=NCOL):
    return len(row) == n

def raw_rows(year):
    path = download.raw(year)
    return filter(has_valid_length, csv_io.yield_rows(path))
...
def dicts0(year):
    return map(as_dict(COLUMNS), raw_rows(year))
...
def dicts(year):
    return map (as_dict(COLUMNS_2), rows(year))
