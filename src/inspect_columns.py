"""
Read column names from TTL files for 2012-2012. 

Create hardcoded column names:
...
TTL_SPEC = {2016: Spec("http://www.gks.ru/opendata/storage/7708234640-bdboo2016/G2016.TTL", 9),
...
def get_format(year):
    return dict(skiprows=TTL_SPEC[year].offset, 
                names=['N', 'line', 'type', 'len'],
                sep=",", 
                encoding='windows-1251')

def get_column_df(year):
    fmt = get_format(year)
    url = TTL_SPEC[year].url
    return pd.read_csv(url, **fmt)

def get_column_names(year):
    df = get_column_df(year)
    assert int(df.loc[0,'N']) == 1
    return list(str(s).strip() for s in df['line'])
...
def generate_colnames():
    ...
    keys_except_section3 = [x for x in d.keys() if not x.startswith("3")]
    this_year = {x: x[0:-1] for x in keys_except_section3 if x.endswith("3")}
    prev_year = {x: x[0:-1]+"_lag" for x in keys_except_section3
                    if x.endswith("4")}
...
if __name__ == "__main__":    
    assert generate_colnames() == Columns.COLUMNS
    
    for year in range(2012, 2017):    
        df = get_column_df(year)
        cn = get_column_names(year)

    
    def screen_colnames():
        years = [2012, 2013, 2014, 2015, 2016]
        cols = {year: get_column_df(year) for year in years}
        for t in years:
            #assert cols[t][0] == 'Наименование'
            if t < years[-1]:
                assert set(cols[t]).difference(cols[t+1]) == set()
    screen_colnames()                
